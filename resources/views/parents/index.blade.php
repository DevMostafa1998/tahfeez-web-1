...
@section('content')
    ...
    <style>
        ...
        .bg-gradient-custom { background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); }
        .premium-card {
        ...
        .progress-circle { transition: stroke-dashoffset 1.5s ease-in-out; }
    ...
    <div class="bg-gradient-custom text-slate-800 min-h-screen pb-10" dir="rtl">
        ...
        <header class="max-w-7xl mx-auto px-4 sm:px-6 py-8 animate__animated animate__fadeIn">
            <div class="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-200">
                <div class="flex items-center gap-5 w-full md:w-auto">
                    <div class="relative group cursor-pointer">
                        ...
                        <div class="absolute -bottom-1 -right-1 bg-emerald-500 w-5 h-5 rounded-full border-2 border-white animate-pulse"></div>
                    </div>
                    <div>
                ...
                <div class="flex flex-col items-end">
                    ...
                    <div class="bg-slate-50 px-4 py-2 rounded-xl border border-slate-200 font-bold text-slate-600 flex items-center gap-2 text-sm">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
            ...
            <section class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border border-white animate__animated animate__fadeInDown">
                <div class="flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
                    ...
                    @if($percentage >= 100)
                        <span class="bg-emerald-100 text-emerald-700 px-4 py-1.5 rounded-full font-bold text-sm flex items-center gap-2">
                            <i class="fas fa-check-circle"></i> أتم الحفظ
                        </span>
                    @else
                        <span class="bg-amber-100 text-amber-700 px-4 py-1.5 rounded-full font-bold text-sm flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
                    <div class="md:col-span-3 flex flex-col items-center justify-center">
                        <div class="relative w-44 h-44">
                            @php
                                ...
                                $strokeColor = $percentage == 100 ? '#10b981' : ($percentage > 50 ? '#3b82f6' : '#f59e0b');
                            @endphp
                            <svg class="w-full h-full transform -rotate-90">
                                <circle cx="88" cy="88" r="58" stroke="#f1f5f9" stroke-width="12" fill="none" />
                                <circle cx="88" cy="88" r="58" stroke="{{ $strokeColor }}" stroke-width="12" fill="none"
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <span class="text-4xl font-black text-slate-800">{{ round($percentage) }}<span class="text-lg">%</span></span>
                    ...
                    <div class="md:col-span-5 space-y-6">
                        <div>
                            <div class="flex justify-between mb-2 text-sm font-bold">
                                ...
                                <span class="text-emerald-600 bg-emerald-50 px-2 rounded">{{ $completedPartsCount }} من 30 جزء</span>
                            </div>
                        ...
                        <div class="p-4 bg-blue-50 rounded-2xl border border-blue-100 hover:bg-blue-100 transition-colors">
                            <p class="text-[10px] text-blue-600 font-bold mb-1 uppercase tracking-wide">آخر سورة تم تسميعها</p>
                            <div class="flex items-center gap-2">
                                <i class="fas fa-book-open text-blue-400 opacity-50"></i>
                                <div>
                                    <p class="font-black text-slate-800 text-lg leading-none">{{ $lastMemo->sura_name ?? '---' }}</p>
                                    @if(isset($lastMemo->date))
                                        <span class="text-[9px] text-slate-400">{{ \Carbon\Carbon::parse($lastMemo->date)->diffForHumans() }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>

                        <div class="p-4 bg-purple-50 rounded-2xl border border-purple-100 hover:bg-purple-100 transition-colors">
                            <p class="text-[10px] text-purple-600 font-bold mb-1 uppercase tracking-wide">توقف عند الآية</p>
                            <div class="flex items-center gap-2">
                    ...
                    <div class="md:col-span-4 h-full">
                        <div class="h-full bg-amber-50 rounded-3xl p-6 border border-amber-100 relative overflow-hidden group">
                            <div class="absolute -right-6 -top-6 w-24 h-24 bg-amber-200 rounded-full opacity-20 group-hover:scale-150 transition-transform duration-500"></div>
                            ...
                            <div class="flex items-center justify-between mb-4 relative z-10">
                                ...
                                @if($lastMemo)
                                    <span class="text-[10px] bg-white px-2 py-1 rounded-lg text-slate-400 font-mono shadow-sm">
            ...
            <section class="bg-white rounded-[2.5rem] p-8 shadow-lg shadow-slate-100 premium-card animate__animated animate__fadeInUp" style="animation-delay: 0.1s">
                ...
                @if(isset($memorizedSurahs) && count($memorizedSurahs) > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($memorizedSurahs as $surah)
                            <div class="p-4 rounded-2xl border transition-all duration-300 hover:shadow-md {{ $surah->is_completed ? 'bg-emerald-50/50 border-emerald-100' : 'bg-white border-slate-100' }}">
                                <div class="flex justify-between items-center mb-3">
                                    <div class="flex items-center gap-2">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold {{ $surah->is_completed ? 'bg-emerald-200 text-emerald-800' : 'bg-slate-200 text-slate-600' }}">
                                            {{ $loop->iteration }}
                                        </div>
                                        <span class="font-bold text-slate-800">{{ $surah->sura_name }}</span>
                                    </div>
                                    @if($surah->is_completed)
                                        <span class="text-[10px] bg-emerald-500 text-white px-2 py-0.5 rounded-full shadow-sm flex items-center gap-1">
                                            <i class="fas fa-check"></i> تم الحفظ
                                        </span>
                                    @else
                                        <span class="text-[10px] bg-amber-500 text-white px-2 py-0.5 rounded-full shadow-sm flex items-center gap-1">
                                ...
                                <div class="flex justify-between items-end text-xs text-slate-500 mb-2 font-bold">
                                    <span>
                                        الآيات: {{ $surah->min_v }} <i class="fas fa-arrow-left text-[8px] mx-1"></i> {{ $surah->max_v }}
                                    </span>
                                ...
                                <div class="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                                    @php
                                        $s_perc = $surah->total_verses > 0 ? ($surah->max_v / $surah->total_verses) * 100 : 0;
                                        if($surah->is_completed) $s_perc = 100;
                                    @endphp
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-8 space-y-8">
                   <section class="bg-white rounded-[2.5rem] p-8 shadow-lg shadow-slate-100 premium-card animate__animated animate__fadeInUp" style="animation-delay: 0.2s">
                    <div class="flex items-center gap-3 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-2xl transition-all duration-300 hover:-translate-y-1 group">
                            ...
                            <div>
                                ...
                                <p class="font-bold text-slate-700 font-mono">{{ $student->phone_number ?? '---' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-2xl transition-all duration-300 hover:-translate-y-1 group">
                            ...
                            <div>
                                ...
                                <p class="font-bold text-slate-700 font-mono">{{ $student->whatsapp_number ?? '---' }}</p>
                            </div>
                        </div>
                    ...
                    <section class="bg-white rounded-[2.5rem] p-8 shadow-lg shadow-slate-100 premium-card animate__animated animate__fadeInUp" style="animation-delay: 0.3s">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <section class="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border-t-8 border-amber-500 premium-card animate__animated animate__fadeInRight">
                        <div class="text-center mb-6">
                           ...
                           @if($teacher)
                            ...
                            <p class="text-sm text-slate-500 font-bold mt-1 bg-slate-100 inline-block px-3 py-1 rounded-full">المحفظ/ة</p>

                            <div class="mt-8 space-y-4">
                                <div class="flex items-center gap-4 p-3.5 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all duration-300 hover:border-amber-300 hover:shadow-md hover:-translate-y-1 group">
                                    <div class="w-12 h-12 rounded-xl bg-amber-50 text-amber-500 flex items-center justify-center text-xl transition-all duration-300 group-hover:bg-amber-500 group-hover:text-white group-hover:rotate-12">
                                        <i class="fas fa-phone-alt"></i>
                                    </div>
                                    <div class="flex-1">
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">رقم الهاتف</p>
                                        <p class="text-lg font-black text-slate-700 font-mono tracking-tighter">{{ $teacher->phone_number }}</p>
                                    </div>
                                </div>

                                @if($teacher->whatsapp_number)
                                    <div class="flex items-center gap-4 p-3.5 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all duration-300 hover:border-emerald-300 hover:shadow-md hover:-translate-y-1 group">
                                        <div class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-500 flex items-center justify-center text-xl transition-all duration-300 group-hover:bg-emerald-500 group-hover:text-white group-hover:-rotate-12">
                                            <i class="fab fa-whatsapp"></i>
                                        </div>
                                        <div class="flex-1">
                                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">واتساب</p>
                                            <p class="text-lg font-black text-slate-700 font-mono tracking-tighter">{{ $teacher->whatsapp_number }}</p>
                                        </div>
                                    </div>
                                @endif
                            </div>
                        @else
                            <div class="mt-8 p-6 rounded-3xl bg-slate-50 border border-dashed border-slate-200 text-center">
                    ...
                    <section class="bg-slate-800 rounded-[2.5rem] p-8 text-white shadow-lg relative overflow-hidden animate__animated animate__fadeInRight" style="animation-delay: 0.1s">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-amber-500 rounded-full filter blur-[50px] opacity-20 -translate-y-1/2 translate-x-1/2"></div>
                        <div class="absolute bottom-0 left-0 w-32 h-32 bg-indigo-500 rounded-full filter blur-[50px] opacity-20 translate-y-1/2 -translate-x-1/2"></div>
                        ...
                        <div class="space-y-6 relative z-10">
                            <div class="flex gap-4 items-start">
                                <div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-amber-400 flex-shrink-0 mt-1">
                            ...
                            <div class="flex gap-4 items-start">
                                ...
                                <div>
                                    ...
                                    <p class="font-bold text-sm leading-relaxed text-slate-300">{{ $student->mosque_address }}</p>
                                </div>
                            </div>
                        </div>
                    </section>
