<style>
    ...
    /* ضبط المسافات لضمان عدم وجود سكرول */
    .compact-body {
    ...
    .compact-body label i {
        ...
        color: #ffc107; /* لون الأيقونات ليناسب طابع المودل */
    }
</style>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-xl-custom">
        <form action="{{ route('user.update', $user->id) }}" method="POST">
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 15px; text-align: right;" dir="rtl">
                ...
                {{-- Header --}}
                <div class="modal-header bg-warning text-dark border-0 py-2 d-flex flex-row-reverse justify-content-between align-items-center">
                ...
                <div class="modal-body compact-body bg-white">
                    ...
                    <div class="row gx-3 mb-4">
                        <div class="col-md-3 text-start">
                            ...
                            <input type="text" name="full_name" class="form-control bg-light border-0 py-2" value="{{ $user->full_name }}" required>
                        </div>
                        <div class="col-md-2 text-start">
                            <label class="fw-bold small"><i class="bi bi-card-text"></i> رقم الهوية</label>
                            <input type="text" name="id_number" class="form-control bg-light border-0 py-2" value="{{ $user->id_number }}" required>
                        </div>
                        <div class="col-md-2 text-start">
                            <label class="fw-bold small"><i class="bi bi-calendar3"></i> تاريخ الميلاد</label>
                            <input type="date" name="date_of_birth" class="form-control bg-light border-0 py-2" value="{{ \Carbon\Carbon::parse($user->date_of_birth)->format('Y-m-d') }}" required>
                        </div>
                        <div class="col-md-2 text-start">
                            <label class="fw-bold small"><i class="bi bi-phone"></i> رقم الجوال</label>
                            <input type="tel" name="phone_number" class="form-control bg-light border-0 py-2" value="{{ $user->phone_number }}" required>
                        </div>
                        <div class="col-md-3 text-start">
                            <label class="fw-bold small"><i class="bi bi-geo-alt"></i> العنوان الحالي</label>
                            <input type="text" name="address" class="form-control bg-light border-0 py-2" value="{{ $user->address }}">
                        </div>
                    </div>

                    {{-- القسم الثاني: البيانات المهنية والقرآنية --}}
                    @if ($user->is_admin == 0)
                        <div class="section-title text-info" style="color: #0dcaf0 !important; border-color: #0dcaf033;">
                        ...
                        <div class="row gx-3 mb-4">
                            <div class="col-md-2 text-start">
                                ...
                                <input type="tel" name="whatsapp_number" class="form-control bg-light border-0 py-2" value="{{ $user->whatsapp_number }}">
                            </div>
                            <div class="col-md-2 text-start">
                                <label class="fw-bold small"><i class="bi bi-mortarboard"></i> المؤهل العلمي</label>
                                <input type="text" name="qualification" class="form-control bg-light border-0 py-2" value="{{ $user->qualification }}">
                            </div>
                            <div class="col-md-2 text-start">
                                <label class="fw-bold small"><i class="bi bi-briefcase"></i> التخصص</label>
                                <input type="text" name="specialization" class="form-control bg-light border-0 py-2" value="{{ $user->specialization }}">
                            </div>
                            <div class="col-md-1 text-start">
                                <label class="fw-bold small"><i class="bi bi-book"></i> الأجزاء</label>
                                <input type="number" name="parts_memorized" class="form-control bg-light border-0 py-2" value="{{ $user->parts_memorized }}">
                            </div>
                            <div class="col-md-3 text-start">
                                <label class="fw-bold small"><i class="bi bi-house-door"></i> اسم المسجد</label>
                                <input type="text" name="mosque_name" class="form-control bg-light border-0 py-2" value="{{ $user->mosque_name }}">
                            </div>
                            <div class="col-md-2 text-start">
                                <label class="fw-bold small"><i class="bi bi-wallet2"></i> رقم المحفظة</label>
                                <input type="text" name="wallet_number" class="form-control bg-light border-0 py-2" value="{{ $user->wallet_number }}">
                            </div>
                            <div class="col-md-2 text-start mt-2">
                                <label class="fw-bold small"><i class="bi bi-map"></i> مكان الميلاد</label>
                                <input type="text" name="birth_place" class="form-control bg-light border-0 py-2" value="{{ $user->birth_place }}">
                            </div>
                            <div class="col-md-2 text-start mt-2">
                    ...
                    <div class="section-title text-success" style="color: #198754 !important; border-color: #19875433;">
                    ...
                    <div class="row gx-3">
                        ...
                        <div class="col-md-2 text-start">
                            ...
                            <select name="category_id" class="form-select bg-light border-0 py-2">
                                @foreach (\DB::table('categorie')->get() as $cat)
                                    <option value="{{ $cat->id }}" {{ $user->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 text-start">
                            ...
                            <input type="password" name="password_confirmation" class="form-control bg-light border-0 py-2" placeholder="تأكيد">
                        </div>
                    </div>
                </div>
